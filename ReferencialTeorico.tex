...
Este capítulo tem como objetivo apresentar o referencial teórico necessário para o desenvolvimento e entendimento deste trabalho, introduzindo conceitos sobre hiper-heurísticas, programação genética, e evolução gramatical. As Seções~\ref{Hiper-Heuristicas},~\ref{Hiper-Heuristicas-Seleção} e~\ref{Hiper-Heuristicas-Geraçao} apresentam o conteúdo necessário para o entendimento sobre hiper-heurísticas, a Seção~\ref{subsection:PG} introduz o conceito e aspectos referente a programação genética, a Seção~\ref{subsubsection:EvolucaoGramatical} apresenta a evolução gramatical e por fim a Seção~\ref{subsubsection:PGasHH} descreve como utilizar programação genética como uma hiper-heurística de geração.
...
\item \textit{Choice Function} (CF): Utiliza uma função que leva em consideração dois fatores: o desempenho das heurísticas assim como quanto tempo se passou desde a última execução de uma heurística.
\item \textit{Greedy} (GR): Realiza as escolhas de maneira gulosa levando em consideração o desempenho das heurísticas.
\item \textit{Tabu Search Based} (TS): Utiliza uma lista tabu para evitar determinadas heurísticas que não apresentaram bom desempenho.
\item \textit{Simulated Annealing} (SA): TODO: Pesquisar para descrever.
\item \textit{Multi-Armed Bandit} (MAB): TODO: Pesquisar para descrever melhor.

\end{itemize}
Além de selecionar heurísticas é necessário um critério de aceitação para definir se uma solução gerada por heurística de baixo nível deverá ser aceita ou não. Diferentes mecanismos têm sido propostos na literatura e estão descritos abaixo:

\begin{itemize}
    \item \textit{All Moves} (AM): Aceita qualquer aplicação de heurística mesmo que seja sem melhora na solução.
    \item \textit{Improvemnt Only} (IO): Aceita apenas aplicações de heurística que melhoraram uma solução.
	\item Monte Carlo (MC): TODO: Pesquisar mais para descrever.
	\item \textit{Great Deluge} (GD): TODO: Pesquisar para descrever.
...
<expr> ::=  <expr> <op> <expr> \alt (<expr> <op> <expr>) \alt <pre-op> (<expr>) \alt <var> \\\
...
Este vetor será utilizado para mapear o cromossomo (genótipo) em um trecho programa (fenótipo) utilizando a gramática BNF. Todos os indivíduos têm o mesmo formato/template apresentado no algoritmo \ref{alg:pseudocodigogrammar}.
...
Nenhuma escolha é necessária para expressão $ \langle var \rangle$ pois existe apenas uma opção $X$. A expressão pode ser reescrita da seguinte maneira: $X \langle op \rangle \langle expr \rangle$
...
Burke et al. \cite{burke2009exploring} também menciona  desvantagens por exemplo: a cada execução da programação genética é encontrada uma melhor heurística, que por se tratar de uma técnica estocástica os resultados podem ser distintos em diferentes execuções. Portanto se faz necessário múltiplas execuções, afim de se obter um melhor conhecimento da qualidade das heurísticas que podem ser produzidas. Outra desvantagem é referente a configuração de parâmetros, que normalmente é encontrada via tentativa e erro.
